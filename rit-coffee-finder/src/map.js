...
const setPitchAndBearing = (pitch = 0, bearing = 0) => {
    // https://docs.mapbox.com/mapbox-gl-js/example/live-update-features
...
const addMarker = (coordinates, title, description, className) => {
    ...
    new mapboxgl.Marker(el).setLngLat(coordinates).setPopup(new mapboxgl.Popup({offset: 25}).setHTML('<h3' + title + '</h3><p>' + description + '/<p>')).addTo(map);
}
